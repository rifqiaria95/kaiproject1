...
$(document).ready(function () {
    $('#Tablejabatan').DataTable({
        ...
        buttons: [
            ...
            {
                text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Tambah Jabatan</span>',
                className: 'add-new btn btn-primary waves-effect waves-light',
                attr: {
                    'data-bs-toggle': 'offcanvas',
                    'data-bs-target': '#offcanvasAddjabatan',
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    let canEdit         = userPermissions.includes("edit_jabatan");
                    let canDelete       = userPermissions.includes("delete_jabatan");
    ...
    $('.card').on('click', '.dt-action-buttons .add-new', function() {
        $('#id').val('');
        $('#formjabatan')[0].reset();
        $('#offcanvasAddjabatanLabel').text('Tambah Jabatan');
    ...
    window.ViewData = function (id) {
        $('#formjabatan .form-control, #formjabatan .form-select').removeClass('is-invalid');
        $('.text-danger.small').text('');

        $.ajax({
            url: `/jabatan/edit/${id}/`, // Pastikan route ini benar
            type: "GET",
            success: function (response) {
                if (response.success) {
                    selectedId = id;
                    $("#id").val(response.jabatan.id);
                    ...
                    $("#offcanvasAddjabatan").offcanvas("show");
                }
            },
            error: function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $("#offcanvasAddjabatan").offcanvas("hide");
